...
Next, we want to output the parsed body, stored in `parts`. `parts` is an array, and as we only sent one file in our HTTP request, our file is at index 0. We also want the binary data of the file. Thus, the output of the function should be `parts[0]`.
