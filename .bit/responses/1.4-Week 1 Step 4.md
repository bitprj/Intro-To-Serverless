...
- [ ]  Commit the function's code in a file named `httptime.js` to the root of the `week1` branch!
## Configuring Azure
...
```
> Note: your parameter must be named `password`, and the function should output the following responses according to whichever case is true:
