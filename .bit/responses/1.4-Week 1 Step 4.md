...
- [ ] ***3:*** Create a trigger that gets content from a request parameter called `password`.
- [ ] ***4:*** Return the content from the request parameter `password` in the body.
- [ ] ***5:*** Place your function URL in a [repository secret](https://docs.github.com/en/actions/reference/encrypted-secrets#creating-encrypted-secrets-for-a-repository) called `HACKERVOICE_ENDPOINT` so we can check your function.
- [ ] ***6:*** Commit the function's code in a file named `HackerVoice/index.js` on the `week1` branch!
...
### 4: Creating a request parameter
