...
We're making a POST request because we are sending the image data to the Face API (we aren't recieving data – that would be a GET request). In order to make this POST request, we'll be using the `node-fetch` package. [What is the node-fetch package?](https://www.npmjs.com/package/node-fetch) Install it!
