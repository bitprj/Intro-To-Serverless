...
**Parameters** play a major role in the actual HTTP request. They are used _to send values in an HTTP request_. In a POST request, like the one above, we use parameters to send data for analysis. In a GET request, data is passed through parameters so that the right data will be sent back to us.
