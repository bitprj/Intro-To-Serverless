...
When running your program on your own computer, the program has no way of accessing Microsoft Azure's function configuration settings, so `process.env` will not work. However, once you deploy the function onto Azure, the function now can "see" the configuration settings, and can use them through `process.env`
</details>
