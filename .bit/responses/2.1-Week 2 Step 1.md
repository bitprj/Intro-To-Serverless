...
Here are the parameters that were in the request URL, which you removed since we will be entering parameters in Postman.
* returnFaceId
* returnFaceLandmarks
...
Read through **Request Parameters** section carefully. How can we specify that we want to get the emotion data?

Notice that all of the parameters are **optional**. That means we don't need to use all of them. Your **request URL** only requires one parameter, with a specific value.

> You should have a "key" `return...` and a "value." You will enter these in Postman
...
:bulb: **_Request Headers_** tell the receiving end of the request what *type* of data is in the body. They can also *authenticate* a request, like a key to a lock!
...
:bulb: **_The body_** of a POST request contains the data you are sending.
