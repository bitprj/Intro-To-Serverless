...
> Note on parameters: Colors are pretty hit or miss; since the Cat API is on the web, it generally adheres to HTML color names. Use [this link](https://www.cknow.com/cms/articles/what-are-the-official-html-color-names-and-hex-codes-3.html) in order to use a wider range colors for your picture! Additionally, the API can take very large words as input for the text, however only **34** characters can be seen on the picture at one time    
