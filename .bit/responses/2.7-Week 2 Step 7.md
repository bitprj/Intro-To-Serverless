...
- [ ] Download the image from the link you parsed in the previous step to make a request to the Face API to receive the predicted age
- [ ] Return the generation from the table below in the function's body based on the age returned from the Face API
- [ ] 🚀 Commit your updated function code to `Song4u/index.js` on the `song4u` branch
